...
一个面向视觉创作者的实时 3D 粒子工作室：把模型或图片变成粒子、制作完整消散与模型变形动画、布置 HDR 和灯光、编辑相机关键帧，最后导出带透明通道的 MOV。

Particle Model Studio is a real-time 3D particle editor for visual creators. Import models or images, design particle dissolves and morphs, preview Gaussian Splats, animate parameters and cameras, and export transparent MOV files.
...
| 动画控制 | 模型动作片段、参数关键帧、相机路径、速度曲线、最终画幅预览 |
| 交互与输出 | MediaPipe 摄像头手势控制；透明 ProRes 4444 MOV 导出 |
...
主要效果参数旁可以直接添加或更新关键帧。粒子数量、尺寸、扩散、生长、消散、Morph、逸散等参数都能随时间变化，不必把所有动作限制在单个固定预设里。

### 10. 相机路径与最终画幅预览

在时间轴上移动相机后点击“K帧”，即可记录位置和朝向。关键帧支持移动、旋转、时间调整和速度曲线；“相机视图”会按最终导出宽高显示画面，减少编辑视角和成片构图不一致的问题。
...
设置宽、高、FPS 和时长后，可直接按相机时间轴渲染 MOV。默认优先编码为带 Alpha 通道的 ProRes 4444（`yuva444p10le`）；若当前 FFmpeg 环境不支持，则自动回退到带透明通道的 qtrle。
...
| 版本 | 包含内容 | 推荐对象 |
| --- | --- | --- |
| **Lite** | 完整编辑器、模型粒子、图片点云、Gaussian Splat 查看、动画与 MOV 导出 | 大多数用户；体积更小，解压即用 |
